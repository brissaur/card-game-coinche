...
exports.giveHandNextPlayer = firestore
    ...
    .onUpdate(event => {
        ...
        if (currentTrick.length > previousTrick.length) {
            ...
            const previousPlayerIdx = previousPlayers.findIndex(
                player => previousPlayerId === player.id
            );
...
exports.fakePlayerTurn = firestore
    .document("tables/{tableId}")
    .onUpdate(event => {
        ...
        const hasCurrentPlayerIdChanged =
            targetPlayerId !== previousData.general.currentPlayerId;
