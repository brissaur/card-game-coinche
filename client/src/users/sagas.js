import { call, put, takeEvery, select } from 'redux-saga/effects';
import {
    getUserName,
    userConnect,
    createFakeUsers,
    createTable,
    joinUsersToTable,
    SET_USERNAME_ACTION_SUCCEED,
    USER_CONNECT_ACTION_SUCCEED,
    USER_CONNECT_ACTION_FAIL,
    SET_USERNAME_ACTION_FAIL,
    SET_USERNAME_ACTION,
    USER_CONNECT_ACTION,
    JOIN_USER_TO_TABLE_ACTION
} from './duck';

function* connectUser() {
    try {
        const username = yield select(getUserName);
        const userDocument = yield call(userConnect, username);
        yield put({
            type: USER_CONNECT_ACTION_SUCCEED,
            id: userDocument.id
        });
        let tableDocument = yield call(createTable);
        console.log('tableDoc', tableDocument.id);
        let usersId = yield call(createFakeUsers);
        // Add current user Id to other ID
        usersId.push(userDocument.id);
        // Join users to table. This should trigger an event on function side to deal card
        yield call(joinUsersToTable, usersId, tableDocument);
    } catch (e) {
        yield put({
            type: USER_CONNECT_ACTION_FAIL,
        });
        throw e;
    }
}

function* setUserName(action) {
    try {
        yield put({
            type: SET_USERNAME_ACTION_SUCCEED,
            username: action.payload.username
        });
    } catch (e) {
        yield put({
            type: SET_USERNAME_ACTION_FAIL,
            message: e.message,
        });
    }
}

/*
 Starts fetchUser on each dispatched `USER_FETCH_REQUESTED` action.
 Allows concurrent fetches of user.
 */
function* usersSaga() {
    yield takeEvery(SET_USERNAME_ACTION, setUserName);
    yield takeEvery(USER_CONNECT_ACTION, connectUser);
    yield takeEvery(JOIN_USER_TO_TABLE_ACTION, connectUser);
}

export default usersSaga;
